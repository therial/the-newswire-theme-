...
class mh_newsdesk_lite_custom_posts extends WP_Widget {
    function mh_newsdesk_lite_custom_posts() {
        $widget_ops = array('classname' => 'mh_newsdesk_lite_custom_posts', 'description' => __('Display posts from any category.', 'mh-newsdesk-lite'));
        $this->WP_Widget('mh_newsdesk_lite_custom_posts', __('MH Custom Posts (lite)', 'mh-newsdesk-lite'), $widget_ops);
    }
		...
		<div class="mh-cp-widget clearfix"><?php
			while ($widget_loop->have_posts()) : $widget_loop->the_post(); ?>
				<article class="cp-wrap cp-small clearfix">
					...
					<p class="entry-meta"><?php echo sprintf(_x('%s', 'post date', 'mh-newsdesk-lite'), '<span class="updated">' . get_the_date() . '</span>'); ?></p>
